<template>
	<div ref="page" :class="{ 'floating-window': true, zIndexTop: moveFlags }" @mouseup.stop.prevent="moveEnd">
		<div class="popover-content__trigger floatBtn" ref="floatBtn" :style="{
			transition: !moveFlags ? 'all 300ms ease' : 'unset',
			transform: `translate3d(${transform.left}px,${transform.top}px,0)`,
		}" @mousedown.stop.prevent="moveStart" @mouseover.stop.prevent="showButton" @click.stop.prevent="toggleMenu">
			<span @mouseleave.stop.prevent="hideButton" :style="{
				left: transform.left < 50 ? '-50px' : 0,
				width: moveFlags ? '50px' : '100px',
			}" class="hover-area"></span>
			<transition name="floating-popover-fade-in-out">
				<div ref="popoverContent" class="floating-popover" :style="menuPosition" v-if="popoverShow">
					<slot>
			...
			<i :style="{ fontSize: `${form.fontSize}px` }" class="iconfont iconnut icon"></i>
			<img class="icon" src="../../assets/images/menu.png" />
		</div>
	</div>
</template>
<script>
